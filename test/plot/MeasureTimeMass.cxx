...
void MeasureTimeMass(std::string nameInFileRoot, int where = 0, std::string run = "B") {
  
  std::string var_Y = "mll";
  int NBIN_Y = 200;
  float MIN_Y = 70.;
  float MAX_Y = 130.;
  ...
  name = Form ("time_Mass_time_%d_%s.root", where, run.c_str());
//   name = Form ("time_time_%d.root", where);
  ...
  std::ofstream myfile;
  name = Form ("time_Mass_time_%d_%s.txt", where, run.c_str());
