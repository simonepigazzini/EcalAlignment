...
*** Step3: Perform alignment 
    - Use bin/EB_Alignment_RotoTraslation_singleSM.cpp (or bin/EB_Alignment_RotoTraslation.cpp) and bin/EE_Alignment_RotoTraslation_singleSM.cpp (or bin/EE_Alignment_RotoTraslation.cpp) scripts to perform alignment. Output of these scripts are two text files containing values of alignment coefficients (∆x, ∆y, ∆z) for each SM and Dee.
    
*** Step4: Validate the alignment results
    - Combine the new alignment coefficients text file with the old one.
    - Perform RE-RECO of the data.
    - Plot ∆η and ∆φ distributions to see if the alignment has resulted in better agreement between data and MC (use ).
